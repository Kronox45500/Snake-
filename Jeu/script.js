...
"use strict";
  ...
  const FIREBASE_CONFIG = {
    apiKey: "AIzaSyCaY7xQN04ijgV1ZFMlQf-ZjW_8hw1_lJg",
    authDomain: "scoreboardsnake-acc1f.firebaseapp.com",
    databaseURL: "https://scoreboardsnake-acc1f-default-rtdb.europe-west1.firebasedatabase.app",
    projectId: "scoreboardsnake-acc1f",
    storageBucket: "scoreboardsnake-acc1f.firebasestorage.app",
    messagingSenderId: "339262221268",
    appId: "1:339262221268:web:1f3f5688cbbac857bac441"
  };
  let firebaseDb = null;
  function firebaseIsConfigured(){ return !!FIREBASE_CONFIG.apiKey && FIREBASE_CONFIG.apiKey.indexOf("YOUR_")!==0; }
  function initFirebase(){
    if(!firebaseIsConfigured()) return false;
    if(typeof firebase === "undefined") return false; // SDK blocked/offline/not loaded
    try{
  ...
  function defaultSave(){
    return {
      ...
      settings:{musicVol:0.15, sfxVol:1},
      uid: genUid(),
      playerName: ""
    };
  }
  ...
  function renderAchievementsOverlay(){
    ...
    $("#statsPanel").classList.toggle("panel-hidden", achTab!=="stats");
    $("#leaderboardPanel").classList.toggle("panel-hidden", achTab!=="leaderboard");
    renderDaily();
    renderAchievementsList();
    if(achTab==="stats") renderStats();
    if(achTab==="leaderboard") renderLeaderboard();
  }
  ...
  function statRow(label, value){
    const row = document.createElement("div");
    row.className = "card";
    row.innerHTML = `<div class="info"><div class="t">${label}</div></div><div class="stat-value">${value}</div>`;
    return row;
  }
  function renderStats(){
    ...
    list.innerHTML = "";
    const s = save.stats;
    const bossIds = [6,12,18,24,30,36];
    const bossesDone = bossIds.filter(id=>save.career.completed.includes(id)).length;
    const rank = rankFor(save.playerLevel);
    list.appendChild(statRow("Rang", rank.emoji+" "+rank.name+" · Niv. "+save.playerLevel));
    list.appendChild(statRow("Meilleur score (Libre)", save.best));
    list.appendChild(statRow("Meilleur score (Chrono)", save.bestChrono));
    list.appendChild(statRow("Fruits mangés au total", s.totalFruits));
    list.appendChild(statRow("Parties jouées", s.totalGames));
    list.appendChild(statRow("Morts au total", s.totalDeaths));
    list.appendChild(statRow("Meilleur combo", "x"+s.bestCombo));
    list.appendChild(statRow("Niveaux de carrière terminés", save.career.completed.length+"/"+LEVELS.length));
    list.appendChild(statRow("Boss vaincus", bossesDone+"/"+bossIds.length));
    list.appendChild(statRow("Tirages gacha", s.gachaPulls));
    list.appendChild(statRow("Serpents possédés", save.ownedSnakes.length+"/"+(SNAKES.length+PREMIUM_SNAKES.length)));
    list.appendChild(statRow("Terrains possédés", save.ownedMaps.length+"/"+MAPS.length));
    list.appendChild(statRow("Succès débloqués", save.achievements.length+"/"+ACHIEVEMENTS.length));
    list.appendChild(statRow("Pièces actuelles", Math.floor(save.coins)));
    list.appendChild(statRow("Cristaux actuels", Math.floor(save.crystals)));
  }
  ...
  $("#lbSaveName").onclick = ()=>{
    const name = $("#lbNameInput").value.trim().slice(0,16) || "Anonyme";
    save.playerName = name;
    persist();
    submitScore("free", save.best);
    submitScore("chrono", save.bestChrono);
    renderLeaderboard();
  };
  function leaderboardCollection(m){ return m==="chrono" ? "leaderboard_chrono" : "leaderboard_free"; }
  function submitScore(m, scoreVal){
    if(!scoreVal || scoreVal<=0) return;
    if(!firebaseIsConfigured()) return;
    if(!firebaseDb && !initFirebase()) return;
    const name = save.playerName || "Anonyme";
    try{
      firebaseDb.ref(leaderboardCollection(m)+"/"+save.uid).set(
        {name, score:Math.round(scoreVal), ts:Date.now()}
      ).catch(()=>{});
    }catch(e){}
  }
  async function renderLeaderboard(){
    document.querySelectorAll(".lb-tab").forEach(t=>t.classList.toggle("active", t.dataset.lbmode===lbMode));
    $("#lbNameInput").value = save.playerName || "";
    const status = $("#lbStatus");
    const list = $("#leaderboardList");
    list.innerHTML = "";
    if(!firebaseIsConfigured()){
      status.textContent = "Classement non configuré pour ce déploiement. Voir le README pour brancher un projet Firebase gratuit.";
      return;
    }
    if(!firebaseDb && !initFirebase()){
      status.textContent = "Impossible de joindre le classement (hors ligne ou bloqué). Réessayez plus tard.";
      return;
    }
    status.textContent = "Chargement…";
    try{
      const snap = await firebaseDb.ref(leaderboardCollection(lbMode)).orderByChild("score").limitToLast(20).once("value");
      const val = snap.val();
      if(!val){ status.textContent = "Aucun score pour l'instant — soyez le premier !"; return; }
      const entries = Object.keys(val).map(uid=>Object.assign({uid}, val[uid]));
      entries.sort((a,b)=>b.score-a.score);
      status.textContent = "";
      entries.forEach((d,i)=>{
        const row = document.createElement("div");
        row.className = "card lb-row"+(d.uid===save.uid ? " me":"");
        row.innerHTML = `<div class="lb-rank">${i+1}</div><div class="info"><div class="t">${d.name||"Anonyme"}</div></div><div class="stat-value">${d.score}</div>`;
        list.appendChild(row);
      });
    }catch(e){
      status.textContent = "Erreur de chargement du classement.";
    }
  }
  ...
  function die(){
    ...
    if(mode==="free" && score > save.best){ save.best = score; submitScore("free", score); }
  ...
  function endChrono(){
    ...
    clearInterval(chaosTimer);
    let newRecord = false;
    if(score > save.bestChrono){ save.bestChrono = score; newRecord = true; submitScore("chrono", score); }
    save.crystals += Math.floor(score/150);
    ensureDaily();
  ...
  const MUSIC_PLAYLIST = ["audio/ReturnToThe8BitPast(1).mp3", "audio/FunWithMy8BitGame(2).mp3", "audio/ArcadeBeat(3).mp3", "audio/TheReturnOfThe8BitEra(4).mp3"];
  let playlistIndex = 0;
  let playlistSkips = 0;
